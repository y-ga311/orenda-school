...
import { validateRegularImportMeta } from "@/lib/examResultRegistration";
import { importRegularExamResults } from "@/lib/examResultRegistration.server";
...
export async function POST(request: Request) {
  ...
  const metaError = validateRegularImportMeta(sessionKey);
  if (metaError) {
  ...
  const result = await importRegularExamResults(supabase, { sessionKey, rows });
  if (!result.ok) {
    return NextResponse.json({ message: result.message }, { status: 400 });
  }

  return NextResponse.json({
    message: `${result.registeredCount}名の定期試験結果を登録しました（新規${result.inserted}件 / 上書き${result.updated}件）。`,
